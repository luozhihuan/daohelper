 ...
 */
public class DAOInsertAction extends AnAction {
    @Override
    ...
    private String getMethodText(String parameterName, PsiClass parameterClass, PsiClass returnClass) {
        ...
        builder.append("org.springframework.jdbc.support.KeyHolder keyHolder = new org.springframework.jdbc.support.GeneratedKeyHolder();");
        builder.append(Constants.BLANK);
        builder.append("db.update(\"INSERT INTO `" + StatementUtils.tableName(StatementUtils.deleteDO(StatementUtils.makeFirstWordLow(parameterClass.getName()))) + "`\" +");
        builder.append(Constants.BLANK);
        ...
        for (int i = 0; i < fieldNameList.size(); i++) {
            ...
            if (i == 0) {
                builder.append("                \" SET `" + StatementUtils.convertToTableFieldName(fieldName) + "` = :"+StatementUtils.convertToTableFieldName(fieldName)+"\" +");
            } else if (i == fieldNameList.size() - 1) {
                builder.append("                \", `" + StatementUtils.convertToTableFieldName(fieldName) + "`= :" + StatementUtils.convertToTableFieldName(fieldName) + "\"");
            } else {
                builder.append("                \", `" + StatementUtils.convertToTableFieldName(fieldName) + "` = :" + StatementUtils.convertToTableFieldName(fieldName) + "\" +");
            }
            builder.append(Constants.BLANK);
        }
        builder.append(", source, keyHolder);");
        builder.append(Constants.BLANK);
        builder.append(parameterName + ".setId(keyHolder.getKey().longValue());");
        builder.append(Constants.BLANK);
